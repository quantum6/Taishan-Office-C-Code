...
{
	return getMetaSize(env,cls,path);
}
...
{
	return drawMetaFile(env, cls, path, width, height, type, crop);
}
/*
 *  函数名称:drawIcon
 *	函数功能:绘制文件的图标
...
{
	return drawIcon(env, cls, path, index);
}
...
{
	return getFileIconByIndex(pEnv, obj, imgPath, filePath, index, fisrtLine, sndLine, fontName);
}
...
{
	return getFileIconNum(pEnv, obj, filePath);
}