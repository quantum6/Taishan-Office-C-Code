...
BOOL CRegistry::CreateKey(LPCTSTR lpSubKey)
{
	...
	HKEY hKey;
	DWORD dw;
	long lReturn=RegCreateKeyEx(m_hKey,lpSubKey,0L,NULL,REG_OPTION_VOLATILE,KEY_READ,NULL,&hKey,&dw);
...
BOOL CRegistry::Open(LPCTSTR lpSubKey)
{
	...
	HKEY hKey;
	long lReturn=RegOpenKeyEx(m_hKey,lpSubKey,0L,KEY_READ,&hKey);
...
BOOL CRegistry::Write(LPCTSTR lpValueName, LPCTSTR lpValue)
{
	...
	long lReturn=RegSetValueEx(m_hKey,lpValueName,0L,REG_SZ,(const BYTE *) lpValue, (DWORD)(wcslen(lpValue)+1));
