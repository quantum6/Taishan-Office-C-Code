                 ...
                 const GdkRectangle& invalidRect)
{
  ...
  NPPluginApp* pApp = (NPPluginApp*)(instanceData->pNPPluginApp);
	...
	pApp->window(GDK_WINDOW_XID(gdkWindow),
        x, y, width, height);

#else

#define TSO "TIO PLUGIN"
  ...
  char display_text[1024*1024] = {0};

  GdkNativeWindow nativeWinId = reinterpret_cast<XID>(window.window);
  sprintf(display_text, "%s-%d", TSO, (int)nativeWinId );
  // paint the UA string
  ...
  pango_layout_set_text(pangoTextLayout, 
                display_text,
                -1);
...
static GdkRegion* computeClipRegion(InstanceData* instanceData)
{
  ...
  for (;;) {
    ...
    GdkRectangle windowRect = { 0, 0, (gint)width, (gint)height };
